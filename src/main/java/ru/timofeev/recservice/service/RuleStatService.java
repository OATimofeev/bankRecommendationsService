...
public class RuleStatService {
    ...
    public void incrementStat(List<RecommendationModel> rules, StatNameEnum stat) {
        rules.forEach(x -> {
            log.info("Increment stats for rule id = {}", x.getId());
            ruleStatRepository.incrementCounterByRuleIdAndStatName(x.getId(), stat);
        });
    }
    ...
    public void create(RecommendationModel createdRule) {
        ...
        ruleStatRepository.save(RuleStatModel
                .builder()
                .rule(createdRule)
    ...
    public void delete(RecommendationModel recommendation) {
        log.info("Was invoked method for delete recommendation product UUID : {}", recommendation.getProductId());
    ...
    public RuleStatsDto getAll() {
        ...
        return RuleStatsDto
                .builder()
                .stats(ruleStatRepository.findAll().stream().map(ruleStatsMapper::getRecommendationDtoFromModel).toList())
                .build();
    }
}